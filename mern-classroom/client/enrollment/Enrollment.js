...
export default function Enrollment ({match}) {
    ...
    /* make a fetch call to read enrollment API to retrieve the details of the enrollment and set to current state */
    useEffect(() => {
    ...
    /* calculating if the total number of completed lessons == total number of lessons, set courseCompleted value */
    /* count value is also stored in the state for rendering in the view */
    ...
    /* handler method for when a specific lesson is selected (drawer item index, or -1 for course overview drawer item) */
    /* sets the drawer value in the state according to the item clicked on the drawer */
    const selectDrawer = (index) => event => {
        setValues({...values, drawer:index})
    }

    /* handler for the lesson's completion button that calls the enrollment update API call */
    /* prepare the values to be sent with the req in the updatedData obj */    
    const markComplete = () => {
                ...
                <CardHeader
                    ...
                    action={
                        <Button 
                            ...
                            color="secondary">{enrollment.lessonStatus[values.drawer].complete? "Completed" : "Mark as complete"}
                        </Button>
                }/>
                <CardContent> 
